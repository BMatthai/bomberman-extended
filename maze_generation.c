...
// void ////printfts(t_set *set1, t_set *set2) {
//
...
// //printf("wall_index : %d, row : %d, col : %d\n",wall_index, row, col);
...
//   int *walls = NULL;
//
//   height_parity = heigh % 2 == 0 ? EVEN : ODD;
//   width_parity = width % 2 == 0 ? EVEN : ODD;
//
//   size_required = (((height - 2 - height_parity) * (width * 2 - width_parity)) / 2) + 1;
//
//   walls = malloc(sizeof(int) * size_required);
//
...
t_set *set_from_value(t_set *sets, int value, int height, int width) {
  ...
  ////printf("Retourne %d d'office\n", value);
  t_set *cur_set = NULL;
  t_element *cur_elt = NULL;

  cur_set = sets;
  printf("On cherche la value %d\n", value);
    ...
    if (is_separator) {
      ...
      if (set1 != NULL && set2 != NULL) {
        ...
        if (is_same_set(set1, set2) == NO) {
          ...
          //printf("%d et %d : same set...\n", cell_side_A, cell_side_B);
        }
      }
      else {
        //printf("Un des deux sets est nuls (%d et %d)\n", cell_side_A, cell_side_B);
      }
    }
    else {
      ////printf("C'est pas un séparateurs %d...\n", wall_index);
    }
  }
  printf("-------\n");
  list_all_sets(sets);
